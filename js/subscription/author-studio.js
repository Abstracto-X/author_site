    ...
    return `<div class="author-studio" data-author-studio-root>
      ...
      <div class="as-grid-2">
        ...
        <section class="card"><div class="section-head"><h2>Published catalog</h2><span class="faint" style="font-size:.78rem">${backendChapters.length} chapters</span></div>
          ${backendChapters.length ? backendChapters.map(({story, chapter}) => `<div class="mgr-row"><span class="mi-ic">${ic(chapter.state === "free" ? "open" : "lock")}</span><div class="mi-body"><div class="mi-t"><span>${e(chapter.title)}</span>${typeof badge === "function" ? badge(chapter.state === "free" ? "free" : "", chapter.state || "member") : ""}</div><div class="mi-s">${e(story.title)} · Ch ${e(chapter.n || "—")} · ${e(chapter.readTime || "?")} min</div></div><div class="mi-acts"><button class="btn sm ghost" data-as-action="draft-from-chapter" data-chapter-id="${attr(chapter.id)}">${ic("copy")}Draft copy</button></div></div>`).join("") : `<div class="empty"><div class="em">${ic("book")}</div><h3>No backend chapters yet</h3><p>Write a draft, then use the Supabase save bridge when your schema is ready.</p></div>`}
        </section>
      </div>
    </div>`;
  }
