...
function feature(name, fallback){
  return Object.prototype.hasOwnProperty.call(FEATURES, name) ? !!FEATURES[name] : !!fallback;
}
function providerEnabled(name){
  return !!PROVIDERS[name];
}
function googleEnabled(){
  return !!AUTH_CONFIG.googleEnabled && feature("enableGoogleOAuth", false);
}
function emailPasswordEnabled(){
  return AUTH_CONFIG.emailPasswordEnabled !== false;
}
function patreonEnabled(){
  return providerEnabled("patreon") && feature("enablePatreonConnect", false);
}
function accessKeysEnabled(){
  return feature("enableAccessKeys", true);
}
function mainArchiveEnabled(){
  return feature("enableMainArchiveLinks", false) && !!MAIN_ARCHIVE_URL;
}
function mainArchiveHref(fragment){
  if (!mainArchiveEnabled()) return "";
  try {
    const url = new URL(MAIN_ARCHIVE_URL, window.location.href);
...
function localDemoMode(){
  ...
  return window.location.protocol === "file:" || host === "localhost" || host === "127.0.0.1" || host === "" || new URLSearchParams(window.location.search).has("demo");
}
function fixtureFallbackAllowed(){
  return feature("enableFixtureFallbackInProduction", false) || (FEATURES.enableLocalDemoReaderFeatures !== false && localDemoMode());
}
function configuredSupabase(){
  const cfg = CONFIG.supabase || {};
  const joined = `${cfg.url || ""} ${cfg.anonKey || ""}`;
  return !!cfg.url && !!cfg.anonKey && !/YOUR_PROJECT_REF|YOUR_SUPABASE|CHANGE_ME|YOUR_DOMAIN/i.test(joined);
}
...
function cleanOAuthCallbackUrl(){
  const url = new URL(window.location.href);
  const routeTarget = url.searchParams.get("sub_route") || oauthCallbackParams().get("sub_route") || AUTH_CONFIG.oauthReturnRoute || "vault";
...
async function initAuth(){
  const client = getSupabase();
  if (!client) { authState.ready = true; authState.error = new Error("Supabase setup required: set supabase.url and supabase.anonKey in site-config.js."); return; }
  try {
...
async function syncProviderEntitlements(){
  if (!patreonEnabled()) throw new Error("Patreon sync is disabled for this site.");
  const client = getSupabase();
  if (!client) throw new Error("Supabase client unavailable.");
...
async function requestPatreonOAuth(){
  if (!patreonEnabled()) throw new Error("Patreon connection is disabled for this site.");
  const client = getSupabase();
  if (!client) throw new Error("Supabase client unavailable.");
...
async function loadBackendLibrary(options = {}){
  ...
  if (!client || backendState.loading) {
    if (!client) {
      backendState.error = new Error("Supabase is not configured. Add your project URL and anon/publishable key to site/js/subscription/site-config.js.");
  ...
  try {
    ...
    if (fixtureFallbackAllowed()) {
      console.warn("Subscription backend library load failed; using local demo fixtures because fixture fallback is allowed for this environment.", err);
      D.STORIES = fixtureStories.map(story => ({ ...story, chapters:(story.chapters||[]).map(ch=>({ ...ch })) }));
...
function backendSetupRequired(){
  return !fixtureFallbackAllowed() && !backendState.loaded;
}
function backendSetupView(){
  ...
  const isLoading = (backendState.loading || !authState.ready) && configured;
  if (isLoading) {
    return `<div class="reader-loading"><div class="reader-spinner"></div><h3>Loading member library</h3><p>Fetching stories, chapter catalog, and access state from Supabase.</p></div>`;
  }
  return `<div class="empty" style="padding-top:90px">
    <div class="em">${I.alert}</div>
    <h3>Subscription site setup required</h3>
    <p>This production reader is configured to use the real Supabase backend only. Demo fixtures are disabled, so no sample stories will be shown.</p>
    <div class="card" style="text-align:left;max-width:640px;margin:16px auto">
      <div style="font-weight:700;margin-bottom:8px">What to check</div>
      <ol class="muted" style="line-height:1.7;margin:0;padding-left:20px">
        <li>Set <code>supabase.url</code> and <code>supabase.anonKey</code> in <code>site/js/subscription/site-config.js</code>.</li>
        <li>Run the SQL files in <code>database/sql/</code> against the new Supabase project.</li>
        <li>Publish at least one story and one chapter, then verify <code>get_chapter_catalog</code>.</li>
      </ol>
      <p class="faint" style="font-size:.78rem;margin:12px 0 0">Current status: ${esc(msg)}</p>
    </div>
  </div>`;
}
...
VIEWS.story = function(){
  ...
  if (s.cast && s.cast.length > 0) {
    ...
    <div class="section">
      ...
      <div class="cast-grid">
        ${s.cast.map(c => {
          const charUrl = mainArchiveHref(c.id ? `gallery/${s.slug}/${c.id}` : `gallery/${s.slug}`);
          const imgHtml = c.img
            ? `<img src="${c.img}" alt="${esc(c.n)}" />`
            : `<div class="cast-fallback">${esc((c.n||"?")[0].toUpperCase())}</div>`;
          return `<${charUrl ? `a class="cast-tile" href="${charUrl}" target="_blank" rel="noopener"` : `div class="cast-tile"`}>
            ${imgHtml}
  ...
  let timelineHtml = "";
  if (s.timeline && s.timeline.length > 0 && mainArchiveEnabled()) {
    const timelineUrl = mainArchiveHref(`timeline/${s.slug}`);
    timelineHtml = `
    <div class="section">
      ...
      <a class="timeline-cta" href="${timelineUrl}" target="_blank">
        ...
        <div class="timeline-cta-body">
          ...
          <div class="timeline-cta-sub">${s.timeline.length} events \u00b7 Open the configured archive for the full chronicle</div>
        </div>
  ...
  ${mainArchiveEnabled()?`<div class="section">
    <div class="card" style="display:flex;gap:12px;align-items:center">
      <span class="faint">${I.external}</span>
      <div style="flex:1"><div style="font-weight:600;font-size:.88rem">Explore in the main archive</div><div class="faint" style="font-size:.76rem">Deep lore, maps, timelines &amp; galleries on the configured archive.</div></div>
      <a class="btn sm ghost" href="${mainArchiveHref()}" target="_blank" rel="noopener">${I.external}</a>
    </div>
  </div>`:""}
  `;
};
...
VIEWS.vault = function(){
  ...
  ${patreonEnabled() || providerEnabled("kofi") || providerEnabled("discord") || providerEnabled("paypal") ? `<div class="section"><div class="section-head"><h2>Providers</h2></div>
    <div class="col-flex">
      ${patreonEnabled()?providerCard("Patreon","patreon",providerConnected,P.tier||null,P.since):""}
      ${providerEnabled("kofi")?providerCard("Ko-fi","kofi",false,null,null,"Coming soon"):""}
      ${providerEnabled("discord")?providerCard("Discord","discord",false,null,null,"Coming soon"):""}
      ${providerEnabled("paypal")?providerCard("PayPal","paypal",false,null,null,"Coming soon"):""}
    </div>
  </div>`:""}

  ${accessKeysEnabled()?`<div class="section"><div class="section-head"><h2>Redeem an access key</h2></div>
    <div class="card">
      ...
      ${fixtureFallbackAllowed()?`<p class="faint" style="font-size:.72rem;margin-top:8px">Local demo key <span class="kbd">DEMO-ACCESS-2026</span></p>`:""}
    </div>
  ...
  <div class="section"><div class="section-head"><h2>Access timeline</h2></div>
    <div class="card"><div class="timeline">
      <div class="tl-item"><div class="when">Just now</div><div class="what">Chapters unlocked after provider sync</div></div>
  ...
  <div class="card" style="display:flex;gap:11px;align-items:center;margin-top:8px"><span class="faint">${I.cog}</span><div style="flex:1"><div style="font-weight:600;font-size:.86rem">Account access</div><div class="faint" style="font-size:.74rem">Supabase auth, catalog RPCs, and entitlement checks are active when configured.</div></div><button class="btn sm" data-nav="/vault">Access hub</button></div>
  `;
};
...
function sheetSignup(){
  return `<span class="close-x" data-act="close-sheet">${I.x}</span><h2>Save your library</h2><p class="sheet-sub">${googleEnabled()?"Use Google for the fastest setup, or create an email login.":"Create an email login for key redemption and future cross-device shelf sync."}</p>
  ${googleEnabled()?`<div class="card" style="margin-bottom:12px"><button class="btn story block" type="button" data-act="google-signin">${I.external}Continue with Google</button></div>`:""}
...
function sheetLock(chId){
  ...
  <div class="col-flex" style="gap:9px">
    ...
    ${patreonEnabled()?(r.state==='expired'?`<button class="btn story block" data-sheet="connect-patreon">${I.sync}Renew via Patreon</button>`:`<button class="btn ${ch.state==='preview'?'ghost':'story'} block" data-sheet="connect-patreon">${I.vault}Connect Patreon</button>`):""}
    ${accessKeysEnabled()?`<button class="btn ghost block" data-sheet="redeem">${I.key}Redeem an access key</button>`:""}
...
function sheetRedeem(){
  if (!accessKeysEnabled()) return `<span class="close-x" data-act="close-sheet">${I.x}</span><h2>Access keys disabled</h2><p class="sheet-sub">This site is not accepting reader access keys right now.</p>`;
  ...
  ${fixtureFallbackAllowed()?`<div class="card" style="margin-top:14px"><div class="eyebrow" style="margin-bottom:6px">Local demo keys</div><div class="faint" style="font-size:.76rem;line-height:1.7"><div><span class="kbd">DEMO-ACCESS-2026</span> \u2014 member preview</div><div><span class="kbd">REVIEWER-2026</span> \u2014 reviewer access</div><div><span class="kbd">WRONG-KEY-9999</span> \u2014 see an error</div></div></div>`:""}`;
}
function sheetConnectPatreon(){
  if (!patreonEnabled()) return `<span class="close-x" data-act="close-sheet">${I.x}</span><h2>Patreon disabled</h2><p class="sheet-sub">This site currently uses ${accessKeysEnabled()?"access keys and manual grants":"manual grants"} for subscription access.</p>${accessKeysEnabled()?`<button class="btn story block" data-sheet="redeem">${I.key}Redeem a key</button>`:""}`;
  return `<span class="close-x" data-act="close-sheet">${I.x}</span><h2>Activate with Patreon</h2><p class="sheet-sub">Patreon proves membership; your ${esc(SITE_NAME)} login saves the library, keys, progress, and provider links.</p>
  ...
  <div class="col-flex" style="gap:9px">${authState.user?`<button class="btn story block" data-act="connect-patreon-go">${I.vault}Continue with Patreon</button>`:(googleEnabled()?`<button class="btn story block" data-act="google-then-patreon">${I.external}Continue with Google, then Patreon</button><button class="btn ghost block" data-sheet="persona">${I.user}Use email instead</button>`:`<button class="btn story block" data-sheet="persona">${I.user}Sign in, then connect Patreon</button>`)}${accessKeysEnabled()?`<button class="btn ghost block" data-sheet="redeem">${I.key}I have a key instead</button>`:""}<a class="btn ghost block" data-nav="/support/wrong-account">${I.user}Wrong account?</a></div>
...
async function connectPatreonGo(){
  if (!authState.user){
    if (googleEnabled()) await signInWithGoogle("connect-patreon");
    else openSheet(sheetPersona);
    return;
  }
  try {
...
function handleAct(act, el){
  switch(act){
    ...
    case "main-archive": if (mainArchiveEnabled()) window.open(mainArchiveHref(), "_blank", "noopener"); else toast("Archive link disabled","Set links.mainArchiveUrl and enableMainArchiveLinks in site-config.js to show archive links.",{icon:"info",ms:3500}); break;
...
VIEWS.studioSettings = function(){
  ...
  ${mainArchiveEnabled()?`<div class="card" style="display:flex;gap:11px;align-items:center"><span class="faint">${I.external}</span><div style="flex:1"><div style="font-weight:600;font-size:.86rem">Connect the main archive</div><div class="faint" style="font-size:.74rem">Sync deep lore & galleries from the configured archive.</div></div><button class="btn sm ghost" data-act="main-archive">${I.external}Connect</button></div>`:""}`;
};
...
VIEWS.home = function(){
  ...
  ${mainArchiveEnabled()?`<p class="faint center" style="font-size:.74rem;margin-top:18px">Deep lore, maps &amp; galleries live in the configured main archive. <button class="btn sm ghost" data-act="main-archive" style="margin-left:6px">${I.external}Open archive</button></p>`:""}`;
};
