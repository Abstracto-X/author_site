...
- Story-level reusable blocks cover writing style, long summary, chapter summary, outline, and freeform scratchpad material. Existing `chapters` and chapter-linked `scratchpads` are referenced directly rather than duplicated.
...
- The workspace uses a narrow library sidebar, dominant central prompt canvas, and compact preset/order rail. The rich context-block editor opens as a dedicated drawer with dirty-state protection while chapter and scratchpad cards hand off to their existing Writer editors.
- Every source card supports source-aware edit, duplicate, and confirmed delete behavior; active-tab select-all/clear controls and global clear are available.
